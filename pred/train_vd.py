...
import vivan_chess_backend
import vivan_chess_backend.vivan

SKIP = 32

#@vivan_chess_backend.logged_main
def main(config_path, name, collection_name):
    ...
    vivan_chess_backend.printWithDate(yaml.dump(cfg, default_flow_style=False))
    ...
    vivan_chess_backend.vivan.ChunkParser.BATCH_SIZE = split_batch_size

    root_dir = os.path.join('models', collection_name, name)
    if not os.path.exists(root_dir):
        os.makedirs(root_dir)
    tfprocess = vivan_chess_backend.vivan.TFProcess(cfg, name, collection_name)
    ...
    if experimental_parser:
        ...
        train_parser = vivan_chess_backend.vivan.ChunkParser(FileDataSrc(train_chunks.copy()),
                shuffle_size=shuffle_size, sample=SKIP, batch_size=vivan_chess_backend.vivan.ChunkParser.BATCH_SIZE)
        train_dataset = tf.data.Dataset.from_generator(
            train_parser.parse, output_types=(tf.string, tf.string, tf.string, tf.string))
        train_dataset = train_dataset.map(vivan_chess_backend.vivan.ChunkParser.parse_function)
    ...
    if experimental_parser:
        ...
        test_parser = vivan_chess_backend.vivan.ChunkParser(FileDataSrc(test_chunks),
                shuffle_size=shuffle_size, sample=SKIP, batch_size=vivan_chess_backend.vivan.ChunkParser.BATCH_SIZE)
        test_dataset = tf.data.Dataset.from_generator(
            test_parser.parse, output_types=(tf.string, tf.string, tf.string, tf.string))
        test_dataset = test_dataset.map(vivan_chess_backend.vivan.ChunkParser.parse_function)
    ...
    num_evals = max(1, num_evals // vivan_chess_backend.vivan.ChunkParser.BATCH_SIZE)
...
def get_latest_chunks(path):
    chunks = []
    vivan_chess_backend.printWithDate(f"found {glob.glob(path)} chunk dirs")
    for d in glob.glob(path):
        vivan_chess_backend.printWithDate(f"found {len(chunks)} chunks", end = '\r')
        chunks += glob.glob(os.path.join(d, '*.gz'))
    vivan_chess_backend.printWithDate(f"found {len(chunks)} chunks total")
    if len(chunks) < 10:
